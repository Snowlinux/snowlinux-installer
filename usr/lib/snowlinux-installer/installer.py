...
class InstallerEngine:
    ...
    def init_install(self, setup):        
        ...
        try:
            ...
            if 'sambashare' and 'lpadmin' in file('/target/etc/group').read():
                ...
                self.do_run_in_chroot("useradd -s %s -c \'%s\' -G sudo,adm,dialout,audio,video,cdrom,floppy,dip,plugdev -m %s" % ("/bin/bash", setup.real_name, setup.username))
    ...
    def do_check_grub(self, our_total, our_current):
        ...
        if os.path.exists("/target/boot/grub/grub.cfg"):
            ...
            for line in grubfh:
                line = line.rstrip("\r\n")
                if("snowlinux.png" in line):
                    found_theme = True
                    print " --> Found Grub theme: %s " % line
                if ("menuentry" in line and "Snowlinux" in line):
                    found_entry = True
